...
def attack_digital_larger(attack_db: str = "LISA"):
    shadow_level = 0.5  # Replace with the desired shadow level
    # position_list, mask_list = load_params()
    save_dir = f'./adv_img/{attack_db}/{int(shadow_level * 100)}'
    try:
    ...
    file_names, orig_imgs, cropped_imgs, cropped_resized_imgs, labels = process_image('larger_images/image_inputs', #/workspace/traffic-diffusion/
                                                            'larger_images/image_annotations', attack_db, 'larger_images/image_masks') #/workspace/traffic-diffusion/
    cnt_attacked = 0
    print(f"Start attacking {len(cropped_resized_imgs)} images")
    for index in range(len(cropped_resized_imgs)):
        mask_type = judge_mask_type(attack_db, labels[index])
...
if __name__ == "__main__":
    ...
    attack_digital_larger(attack_db)