...
from attacks.ShadowAttack.utils import SmoothCrossEntropyLoss
from attacks.ShadowAttack.utils import draw_shadow
from attacks.ShadowAttack.utils import shadow_edge_blur
from attacks.ShadowAttack.utils import judge_mask_type
...
def training(training_model, train_data, train_labels, test_data, test_labels, adv_train=False):
    ...
    torch.save(training_model.state_dict(),
               # f'{MODEL_PATH}/{"adv_" if adv_train else ""}model_lisa.pth')
...
def test_model(adv_model=False):
    ...
    trained_model.load_state_dict(
        # torch.load(f'{MODEL_PATH}/{"adv_" if adv_model else ""}model_lisa.pth',
...
def test_single_image(img_path, ground_truth, adv_model=False):
    ...
    msg = f'Correct: {index==ground_truth}\nPredict: {index} Confidence: {confidence*100}%'
    return index, index == ground_truth, msg
