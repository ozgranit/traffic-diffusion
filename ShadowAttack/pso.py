...
class PSO:
    ...
    def prepare_single_image_with_no_transformation_for_physical_atatck(self, image, shadow_position, pos_list, shadow_level, shadow_edge_blur_coefficient):
    ...
    def fit_fun(self, position, **parameters):
        ...
        if self.physical and not self.with_EOT:
            ...
            img = self.prepare_single_image_with_no_transformation_for_physical_atatck(self.image, position, self.coord, self.coefficient, shadow_edge_blur_coefficient)
            if self.generated_images is not None:
                all_imgs_list = [img.to(device)]
                for generated_img in self.generated_images:
                    gen_img = self.prepare_single_image_with_no_transformation_for_physical_atatck(generated_img, position, self.coord, self.coefficient, shadow_edge_blur_coefficient).to(device)
