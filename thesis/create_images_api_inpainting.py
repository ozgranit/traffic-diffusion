...
URL_SUFFIX = '/stable-diffusion-xl/inpaint'

INPUT_FOLDER = r'/workspace/traffic-diffusion/datasets/imags_with_shadow/input_large_src_with_shadow' #'api_input/'
OUTPUT_FOLDER = r'/workspace/traffic-diffusion/datasets/imags_with_shadow/output_api_inpainting'   #'api_output/'
...
def load_images(folder_path, images_filter=None):
    ...
    for filename in os.listdir(folder_path):
        ...
        if img is not None:
            original_images[filename.split('.')[0]] = [img, mask]
    ...
    for index, (filename, images) in enumerate(original_images.items()):
        ...
        for prompt_desc, cur_prompt in prompt_getter.items():
            ...
            while retry_count < max_retries:
                ...
                # convert the image-mask to base64 encoding
                _, buffer_mask = cv2.imencode('.jpg', mask)
                base64_image_mask = base64.b64encode(buffer_mask).decode('utf-8')

                data['model'] = 'stable-diffusion-xl-v1-0'
                data['image'] = base64_image
                data['mask_image'] = base64_image_mask
                data['prompt'] = cur_prompt
                data['negative_prompt'] = NEGATIVE_PROMPT
                data['strength'] = 0.3  #0.7
                data['steps'] = 50
                data['guidance'] = 8
                data['seed'] = seed
