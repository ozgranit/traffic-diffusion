...
import time
import utils_rfla
from RFLA.diffusion_generated_imgs import DiffusionImages
from RFLA.traffic_data import TrafficData
from ShadowAttack.lisa_model import LisaModel
        ...
        for i in range(self.size):
            ...
            if len(success_indicator_index) >= 1:
                ...
                if self.best_pop_on_src_and_diffusoin_count_imgs_success == 0:
                    ...
                    if self.best_pop_on_src_and_diffusoin_count_imgs_success > 0:
                        print(
                            f"Initialize Success on {self.best_pop_on_src_and_diffusoin_count_imgs_success} images: g_fitness: {self.g_best_fitness}, g_fitness_real: {self.sg_best_fitness}, p_fitness: {self.p_best_fitness[i]}, prediction: {indices[0]}, probability: {fitness_score[0]}")
                    else:
                        print(
            ...
            if len(success_indicator_index) >= 1:
                ...
                if diffusion_imgs is not None:
                    ...
                    if succeeded_pops.max() > self.best_pop_on_src_and_diffusoin_count_imgs_success:
                        ...
                        print(f"【{itr}/{self.max_iter}】Success: g_fitness: {self.g_best_fitness}, p_fitness: {self.p_best_fitness[i]}, prediction: {indices[success_indicator.cpu().data.numpy()][0]}, probability: {fitness_score[success_indicator.cpu().data.numpy()][0]}")
                ...
                if self.best_pop_on_src_and_diffusoin_count_imgs_success > 0:
                    print(
                        f"【{itr}/{self.max_iter}】Success on {self.best_pop_on_src_and_diffusoin_count_imgs_success} images: g_fitness: {self.g_best_fitness}, g_fitness_real: {self.sg_best_fitness}, p_fitness: {self.p_best_fitness[i]}, prediction: {indices[0]}, probability: {fitness_score[0]}")
                else:
                    print(f"【{itr}/{self.max_iter}】Failed: g_fitness: {self.g_best_fitness}, g_fitness_real: {self.sg_best_fitness}, p_fitness: {self.p_best_fitness[i]}, prediction: {indices[0]}, probability: {fitness_score[0]}")
...
if __name__ == "__main__":
    ...
    experiment_dir = f'RFLA/larger_images_experiments/physical_attack_RFLA_{args.attack_db}_shape-{args.shape_type}_maxIter-{args.max_iter}_tmp'  # _EOT-{with_EOT}
    ...
    asr_with_diffusion = pso.run_pso_with_diffusion_imgs(file_names, orig_imgs, cropped_imgs, cropped_resized_imgs, labels, bbx, masks_cropped, run_pso_with_diffusion_imgs=True)
